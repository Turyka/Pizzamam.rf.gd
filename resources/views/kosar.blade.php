...
<body>
    ...
    @if (session('kosar') > 0)
        @foreach ($kosar as $kosaritem)
            <x-kosar-card :kosaritem="$kosaritem" />
        @endforeach
    @endif
    <h2 id="Totalprice" class="text-center"> {{ $Totalprice }} Ft</h2>
    <h2 class="text-center listingCount"> {{ $listingCount }} db</h2>
    <form action="{{ route('kosar.vege') }}" method="POST">
        @csrf
        <h4 class="text-center">
            @if (session('kosar') > 0)
                @foreach ($kosar as $kosaritem)
                    <input type="hidden" name="Pizza_darab_nev[]"
                        value="{{ $kosaritem['quantity'] }}, {{ $kosaritem['cim'] }}">
                @endforeach
            @endif
            <input type="hidden" name="Teljes_osszeg" value="{{ $Totalprice }}">
            <input type="hidden" name="Ossz_darab" value="{{ $listingCount }}">
        </h4>
